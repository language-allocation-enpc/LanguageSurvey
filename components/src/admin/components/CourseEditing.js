...
class CourseEditing extends Component {
  ...
  componentDidMount() {
    ...
    axios.get(url + "creneaux/", { withCredentials: false }).then(res => {
      let creneaux = res.data.result;
      let new_state = this.state;
      new_state.creneaux = creneaux;
      this.setState(new_state);
    });
    axios.get(url + "courses/", { withCredentials: false }).then(res => {
      let data_serv = res.data.result;
      let new_state = this.state;
      new_state.data = data_serv;
      this.setState(new_state);
    });
  ...
  render() {
    ...
    return <div className="main-container">{page}</div>;
  }
}
...
class Cours extends Component {
  constructor(props) {
    super(props);
    this.state = {
      current_filters: { language: "", schedule: "" },
  ...
  render() {
    ...
    for (let index = 0; index < newdata.length; index++) {
      let quit_button = (
        <img width="50%" height="15%" src={suppr_button} alt="" onClick={() => this.props.confirm(newdata[index].id)} />
      );
      let course_info = [];
      course_info.push(
        <div className="cours">
          ...
          <div className="champs">
            <div className="caractéristiques">langue : </div>
            <div className="attribut">{newdata[index].language}</div>
          </div>
          <p></p>
          {newdata[index].creneaux.map(id => {
            return (
              <p>
                <div className="champs">
                  <div className="caractéristiques"> horaires : </div>
                  <div className="attribut">{L_creneaux[id].day}</div>
      ...
      let change_button = (
        <img width="50%" height="15%" src={edit_button} alt="" onClick={() => this.props.edit_course(index)} />
      );
      course_info.push(
        <div className="up-right_button">
    ...
    for (let criterion in this.props.criteriaWithOptions) {
      ...
      criteria_selection.push(
        <div className="course-list-filter-option" key={criterion}>
          <label>
            {criterion === "language" ? (
              <div className="texte">Langue : </div>
            ) : criterion === "schedule" ? (
              <div className="texte">Horaire : </div>
            ) : (
              "erreur"
            )}
          </label>
...
class Edit extends Component {
  ...
  render() {
    ...
    change_course.push([
      <h1 className="Title">Modifier le cours {this.state.data[id].name}</h1>,
      <label className="cat">Horaires : </label>,
      <div>
        {this.state.data[id].creneaux.map(cren => {
          return (
            <select
            ...
            >
              {creneaux_op.map(creneaux => {
                return (
                  <option value={creneaux.value} id={this.state.data[id].creneaux.indexOf(cren)}>
                    {creneaux.label}{" "}
                  </option>
                );
              })}
      ...
      <p>
        <label className="cat">Nom : </label>
        <input name="changer_nom" type="text" value={this.state.data[id].name} onChange={this.handleInputChange} />{" "}
      </p>,
      <label className="cat">
        Changer la capacité : <br />
      </label>,
      <p>
        <label className="cat">Minimum : </label>
        <input
          ...
          onChange={this.handleInputChange}
        />
        <label className="cat"> Maximum : </label>
        <input
...
class Add extends Component {
  ...
  render() {
    ...
    creneaux_op[0].label = "hors creneaux";
    add_course.push([
      <h1 className="Title">Ajouter cours</h1>,
      <label className="cat">nombre de créneaux</label>,
      <select
      ...
      <div>
        {this.state.course.creneaux.map(cren => {
          return (
            <select
            ...
            >
              {creneaux_op.map(creneaux => {
                return (
                  <option value={creneaux.value} id={this.state.course.creneaux.indexOf(cren)}>
                    {creneaux.label}{" "}
                  </option>
                );
              })}
      ...
      <p>
        <label className="cat">nom : </label>
        <input name="ajouter_nom" type="text" value={this.state.course.name} onChange={this.handleInputChange} />{" "}
      </p>,
      <label className="cat">changer la capacité :</label>,
      <p>
        <label className="cat">minimum :</label>
        <input
