...
/*function parse(json_file_name) {
  ...
  for (let i_student = 0; i_student < data.length; i_student++) {
    ...
    if (nb_languages === 1) {
      result[i_student].vows = generate_combinations(
        sortings_dic[languages_name[0]],
        parseInt(wanted_organization[languages_name[0]])
      );
    } else {
      result[i_student].vows = mix(
        generate_combinations(sortings_dic[languages_name[0]], parseInt(wanted_organization[languages_name[0]])),
        generate_combinations(sortings_dic[languages_name[1]], parseInt(wanted_organization[languages_name[1]]))
      );
    }
  }
  return result;
...
function vowGenerator(
  number_english_courses,
  number_other_courses,
  ranking_english_courses,
  ranking_other_courses,
  schedules
) {
  let raw_vow_list = mix(
    generate_combinations(ranking_english_courses, number_english_courses),
    generate_combinations(ranking_other_courses, number_other_courses)
  );
  let checked_vow_list = raw_vow_list.filter(vow => {
    return vowIsValid(vow, schedules);
  });
  checked_vow_list.sort((vow1, vow2) => {
    return vow1.weight - vow2.weight;
  });
