...
class QuestionInstructions extends Component {
  render() {
    return <h2 className="question-instructions"> {this.props.text}</h2>;
  }
}
  ...
  render() {
    ...
    return (
      <div className="question-footer">
        {" "}
        <div className="question-footer-left"> {left_button}</div>
        <div className="question-footer-right">{right_button}</div>
      </div>
    );
  }
}
...
class CourseBox extends Component {
  render() {
    ...
    let current_course_schedules_list = this.props.course.creneaux.map(schedule_index => (
      <p key={schedule_index}>
        {schedules[schedule_index].day +
          (schedules[schedule_index].day === "hors-créneaux"
            ? ""
            : " " + schedules[schedule_index].begin + "-" + schedules[schedule_index].end)}
      </p>
    ));
    return (
      <Card className="course-box">
        {" "}
        <div>
          <h2>{this.props.course.name}</h2> <h3>{this.props.course.language}</h3>
          {current_course_schedules_list}
        </div>{" "}
        <div className="course-box-buttons">{this.props.buttons}</div>
      </Card>
    );
  }
}
...
class ChangeStepButton extends Component {
  render() {
    return (
      <Button className="change-step-button" onClick={this.props.onClick}>
        {this.props.text}
      </Button>
    );
  }
...
class FinalButton extends Component {
  render() {
    return (
      <Button className="final-button" onClick={this.props.onClick}>
        {this.props.text}
      </Button>
    );
  }
  ...
  render() {
    return (
      <div className="error-message">
        <img src={error_panel} alt="error_panel" style={{ height: "5vh" }} />
        {this.props.text}
      </div>
    );
  }
