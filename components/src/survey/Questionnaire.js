...
import { FinalButton, QuestionFooter, QuestionInstructions, WarningMessage } from "./components/utils";
...
class Questionnaire extends Component {
  ...
  componentDidMount() {
    axios.get(url + "courses/", { withCredentials: false }).then(
      result => {
    ...
    axios.get(url + "creneaux/", { withCredentials: false }).then(
      result => {
        let newState = Object.assign({}, this.state);
  ...
  buildAvailableCourses = (answers, data) => {
    ...
    if (answers.year === "1A") {
      available_courses = available_courses.filter(course => {
      ...
      available_courses = available_courses.filter(course => {
        ...
        for (let i = 0; i < course.creneaux.length; i++) {
          available_for_2A3A =
            available_for_2A3A &&
            (data.schedules[course.creneaux[i]].type.split(", ").includes("2A") ||
              data.schedules[course.creneaux[i]].type.split(", ").includes("3A"));
        }
  ...
  getStepIndex = () => {
    return this.state.step_index;
  };

  getUserName = () => {
    return this.state.data.user_name;
  };
  ...
  handleStepChange = direction => {
    if (direction === "previous" && this.state.step_index > 0) {
      ...
      if (this.state.step_index + 1 === this.state.step_list.length - 1) {
  ...
  getStepList = () => {
    return this.state.step_list;
  };

  setStepList = new_step_list => {
    this.setState({ step_list: new_step_list });
  };
  ...
  updateVows = () => {
    ...
    let user_vows = vowGenerator(
      answers.number_english_courses,
      answers.number_other_courses,
      answers.english_courses_ranking,
      answers.other_courses_ranking,
  ...
  sendAnswers = () => {
    axios
      .post(
        url + "users/students/vows/" + this.state.data.token,
        { vows: this.state.data.user_vows },
  ...
  render() {
    ...
    if (current_step_name === "initial_questions") {
      ...
      let list_english_courses = available_courses.filter(function(value, index, arr) {
        return value.language === "Anglais";
      });
      step = (
        <CourseRankingQuestion
      ...
      let list_other_courses = available_courses.filter(function(value, index, arr) {
        return value.language !== "Anglais";
      });
      step = (
        <CourseRankingQuestion
    ...
    return <Paper className="questionnaire">{step}</Paper>;
  }
}
      ...
      <div className="question">
        ...
        <div className="question-content">
          <div className="final-question-container">
            {this.props.getAnswersAreSent() ? (
              <div className="final-buttons-box">Vos réponses ont bien été envoyées.</div>
            ) : (
              [
                enough_vows
                  ? null
        ...
        {this.props.getAnswersAreSent() ? null : (
          <QuestionFooter
