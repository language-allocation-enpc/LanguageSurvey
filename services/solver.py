...
from random_data import generate_random_course_list, generate_random_population
...
def solve(COURSES, STUDENTS):
    ...
    # Objective function
    assignment_model.min(sum(STUDENTS[s].vows[v].weight * student_gets_vow[s, v] for s, v in SxV))
    ...
    for course in C:
        ...
        sum(
            student_gets_vow[s, v] * int(COURSES[course] in STUDENTS[s].vows[v].courses) for s, v in SxV
        ) == course_headcount[course]
